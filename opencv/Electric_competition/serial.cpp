...
void Uart::GetMode(int& fd, GroundChassisData& data)
{
    ...
    if (bytes == 0)
        return;
    bytes = read(fd, rdata, 4);

    if (rdata[0] == 0xAA && rdata[3] == 0XBB)  //&& Verify_CRC8_Check_Sum(rdata,6)
    {
        data.color = rdata[1];

        ///< pitch
        data.gain_pitch.c[0] = rdata[2];
        //        data.gain_pitch.c[1] = rdata[3];
        //        data.gain_pitch.c[2] = rdata[4];
        //        data.gain_pitch.c[3] = rdata[5];

        //        ///< yaw
        //        data.gain_yaw.c[0] = rdata[6];
        //        data.gain_yaw.c[1] = rdata[7];
        //        data.gain_yaw.c[2] = rdata[8];
        //        data.gain_yaw.c[3] = rdata[9];
    }
}
/**
 * @brief 串口发送数据
...
void Uart::TransformTarPos(int& fd, const HostComputerData& data)
{
    Sdata[0] = 0xA5;
    // 0 ： 左转  --  1 ： 右转  、寻线修正
    Sdata[1] = 5 /*data.Direction*/;
    Sdata[2] = 999 /*data.Turn*/;          // 是否开启大转向  0 - 左转 1 - 右转
    Sdata[3] = data.Number /*data.Turn*/;  // 是否开启大转向  0 - 左转 1 - 右转
...
{
    if (open("/dev/ttyUSB0", O_RDWR | O_NONBLOCK) == -1)
    {
        fdcom = open("/dev/ttyUSB1", O_RDWR | O_NONBLOCK);  // O_NOCTTY | O_NDELAY
    }
    else
    {
