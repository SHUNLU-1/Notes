...
try
{
    // Create and initialize GUI related objects  创建初始化窗口

    // ImGui写法
    // window app(1280, 720, "CPP - Align Example"); // Simple window handling
    // ImGui_ImplGlfw_Init(app, false);              // ImGui library intializitio
    ...
    // 最好创建统一尺寸 默认有差别
    pipe_config.enable_stream(RS2_STREAM_DEPTH, 1024, 768, RS2_FORMAT_Z16, 30);
    ...
    */
    ...
    while (cv::getWindowProperty(depth_win, WND_PROP_AUTOSIZE) && getWindowProperty(color_win, WND_PROP_AUTOSIZE)) // Application still alive?
    {
