...
float get_depth_scale(rs2::device dev)
{
    ...
    {
        // Check if the sensor if a depth sensor 查看是否为深度相机
...
int main()
{
    ...
    //直接获取从深度摄像头坐标系到彩色摄像头坐标系的欧式变换矩阵(外参)
    auto extrinDepth2Color = depth_stream.get_extrinsics_to(color_stream);

    while (getWindowProperty(depth_win, WND_PROP_AUTOSIZE) && getWindowProperty(color_win, WND_PROP_AUTOSIZE)) // Application still alive?
    {
        //堵塞程序直到新的一帧捕获
