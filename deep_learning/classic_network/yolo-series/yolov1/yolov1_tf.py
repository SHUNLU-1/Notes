...
class Yolo(object):
	...
	def __init__(self,weights_file,input_image,verbose=True):
		...
		'''边界框的中心坐标xy——相对于每个cell左上点的偏移量   [self.B, self.S, self.S]--->[1, 2, 0]'''
		...
		self.detect_from_file(image_file=input_image) 
	...
	def _build_net(self):
		...
		''' 输入、输出用占位符，因为尺寸一般不会改变  None表示不确定，为了自适应batchsize'''
	...
	'''对应【1】:定义conv/maxpool/flatten/fc层  卷积层:x输入；id:层数索引；num_filters:卷积核个数；filter_size:卷积核尺寸；stride:步长'''
	def _conv_layer(self,x,id,num_filters,filter_size,stride):
		...
		weight = tf.Variable(tf.random.truncated_normal([filter_size,filter_size,in_channels,num_filters],mean=0.0,stddev=0.1))
	...
	def _maxpool_layer(self,x,id,pool_size,stride):
		output = tf.compat.v1.layers.max_pooling2d(inputs=x,
										 pool_size=pool_size,
										 strides=stride,
										 padding='SAME')
		if self.verbose:
    ...
    tran_x  [batch,行,列,通道数channels] -> [batch,通道数channels,列,行]
    nums    计算的是总共的神经元数量，第一个表示batch数量所以去掉
	...
	def _fc_layer(self,x,id,num_out,activation=None):
		...
		'''均值为0标准差为0.1的正态分布，初始化权重w；shape=行*列*通道数*卷积核个数'''
		weight = tf.Variable(tf.random.truncated_normal(shape=[num_in,num_out],mean=0.0,stddev=0.1))
		bias = tf.Variable(tf.zeros(shape=[num_out,])) # 列向量
		output = tf.compat.v1.nn.xw_plus_b(x,weight,bias)
	...
	def _build_detector(self):
		...
		'''将x，y转换为相对于图像左上角的坐标 w，h的预测是平方根乘以图像的宽度和高度'''
		...
		'''NMS (不区分不同的类别) 中心坐标+宽高box (x, y, w, h) -> xmin=x-w/2 -> 左上+右下box (xmin, ymin, xmax, ymax)，因为NMS函数是这种计算方式'''
	...
	def _load_weights(self,weights_file):
		...
		'''导入权重 初始化  saver.restore导入/saver.save保存'''
